...
class clsNumber 
{
	 ...
	 *
	 *     echo 2, clsNumber::ordinal(2);   // "2nd"
	 *     echo 10, clsNumber::ordinal(10); // "10th"
	 *     echo 33, clsNumber::ordinal(33); // "33rd"
	 *
	 ...
	 *     echo clsNumber::format(1200.05, 2);
	 *
	 *     // In English, "1,200.05"
	 *     // In Spanish, "1.200,05"
	 *     // In Portuguese, "1.200.05"
	 *     echo clsNumber::format(1200.05, 2, TRUE);
	 *
	 ...
	 * clsNumber::$byte_units
	 *
	 *     echo clsNumber::bytes('200K');  // 204800
	 *     echo clsNumber::bytes('5MiB');  // 5242880
	 *     echo clsNumber::bytes('1000');  // 1000
	 *     echo clsNumber::bytes('2.5GB'); // 2684354560
	 *
	...
	public static function bytes($size)
	{
		...
		$accepted = implode('|', array_keys(clsNumber::$byte_units));
		...
		if ( ! preg_match($pattern, $size, $matches))
			throw new expCore('The byte unit size, "'.$size.'", is improperly formatted.');

		// Find the float value of the size
		$size = (float) $matches[1];

		// Find the actual unit, assume B if no unit specified
		$unit = clsArray::get($matches, 2, 'B');

		// Convert the size into bytes
		$bytes = $size * pow(2, clsNumber::$byte_units[$unit]);
