...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
    function matthewruddy_image_resize( $url, $width = 150, $height = 150, $crop = true, $retina = true ) {

        global $wpdb;

        if ( empty( $url ) )
            return new WP_Error( 'no_image_url', __( 'No image URL has been entered.','aquilo' ), $url );
    ...
    function matthewruddy_image_resize( $url, $width = 150, $height = 150, $crop = true, $retina = false ) {
        ...
        if ( !file_exists( $dest_file_name ) ) {
            ...
            if ( !$size )
                return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".','aquilo'), $file_path );
            ...
            if ( IMAGETYPE_GIF == $orig_type ) {
                if ( !imagegif( $new_image, $dest_file_name ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)','aquilo') );
            }
            elseif ( IMAGETYPE_PNG == $orig_type ) {
                if ( !imagepng( $new_image, $dest_file_name ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).','aquilo') );
            }
            else {
                ...
                if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 90 ) ) )
                    return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).','aquilo' ) );
            ...
            if ( !$new_size )
                return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize','aquilo' ), $dest_file_name );
