...
define('_DEBUG_INLINEBTN_ELEGANT', FALSE);



class cunjoshare_InlineButtons_controller extends absController

{
    ...
    public function action_DisplayBar($post_content = NULL)
    {
        if($post_content == NULL)
            $post_content = (string)$this->get_hmvc_params('content');   
        return $post_content;
        ...
        if($settings['visibility_settings']['is_active'] == TRUE OR (isset($_GET['cunjo']) && $_GET['cunjo'] == $this->layout))
        {
           ...
           if(is_home() OR is_front_page()){
                ...
                if($settings['visibility_settings']['on_home'] == TRUE){
                   ...
                   echo $this->output_bar($post_content);
                }else{
                   if(_DEBUG_INLINEBTN_ELEGANT){
    ...
    private function output_bar($post_content)
    {
        ...
        if($widget_settings['visibility_settings']['placement'] == 'above')
            echo str_replace('id="cunjo_widget_'.$this->layout.'"', 'id="cunjo_widget_'.$this->layout.'_1_' . uniqid() . '"', $anchor) . "<div style='clear:both'></div><br />" . $post_content;
        else if($widget_settings['visibility_settings']['placement'] == 'below')
            echo $post_content . "<br />" . str_replace('id="cunjo_widget_'.$this->layout.'"', 'id="cunjo_widget_'.$this->layout.'_1_' . uniqid() . '"', $anchor);
        else if($widget_settings['visibility_settings']['placement'] == 'both')
            echo str_replace('id="cunjo_widget_'.$this->layout.'"', 'id="cunjo_widget_'.$this->layout.'_1_' . uniqid() . '"', $anchor) . "<br />" . $post_content . str_replace('id="cunjo_widget_'.$this->layout.'"', 'id="cunjo_widget_'.$this->layout.'_2_' . uniqid() . '"', $anchor);
        else echo $post_content;

        //echo $anchor . $post_content;
