<?php 
class clsHTTP_Header extends ArrayObject 
{
	...
	public static function accept_quality(array $parts)
	{
		...
		foreach ($parts_keys as $key)
		{
			...
			{
				$parsed[$value] = (float) clsHTTP_Header::DEFAULT_QUALITY;
			}
			else
			{
				$quality = $quality[2];
	...
	public static function parse_accept_header($accepts = NULL)
	{
		...
		if ($accepts === NULL)
			return array('*' => array('*' => (float) clsHTTP_Header::DEFAULT_QUALITY));

		// Parse the accept header qualities
		$accepts = clsHTTP_Header::accept_quality($accepts);
	...
	public static function parse_charset_header($charset = NULL)
	{
		if ($charset === NULL)
		{
			return array('*' => (float) clsHTTP_Header::DEFAULT_QUALITY);
		}

		return clsHTTP_Header::accept_quality(explode(',', (string) $charset));
	}
	...
	public static function parse_encoding_header($encoding = NULL)
	{
		...
		elseif ($encoding === '')
		{
			return array('identity' => (float) clsHTTP_Header::DEFAULT_QUALITY);
		}
		else
		{
			return clsHTTP_Header::accept_quality(explode(',', (string) $encoding));
		}
	}
	...
	public static function parse_language_header($language = NULL)
	{
		...
		$language = clsHTTP_Header::accept_quality(explode(',', (string) $language));
	 ...
	 *     $response->headers('Cache-Control', clsHTTP_Header::create_cache_control($cache_control);
	 *
	 ...
	 *     if ($cache_control = clsHTTP_Header::parse_cache_control($response->headers('cache-control')))
	 *     {
	...
	public function accepts_at_quality($type, $explicit = FALSE)
	{
		...
		if ($this->_accept_content === NULL)
		{
			...
			$this->_accept_content = clsHTTP_Header::parse_accept_header($accept);
		}
		...
		if (strpos($type, '/') === FALSE)
		{
			...
			$mime =  WPFuel::load(CORE_PATH . 'config' . DS . 'mimes.php');
	...
	public function accepts_charset_at_quality($charset)
	{
		if ($this->_accept_charset === NULL)
		{
			if ($this->offsetExists('Accept-Charset'))
			{
				...
				$this->_accept_charset = clsHTTP_Header::parse_charset_header($charset_header);
			}
			else
			{
				$this->_accept_charset = clsHTTP_Header::parse_charset_header(NULL);
			}
		}
	...
	public function accepts_encoding_at_quality($encoding, $explicit = FALSE)
	{
		if ($this->_accept_encoding === NULL)
		{
			...
			$this->_accept_encoding = clsHTTP_Header::parse_encoding_header($encoding_header);
		}
		...
		if ($explicit === FALSE)
		{
			...
			{
				return (float) clsHTTP_Header::DEFAULT_QUALITY;
			}
		}
	...
	public function accepts_language_at_quality($language, $explicit = FALSE)
	{
		if ($this->_accept_language === NULL)
		{
			...
			$this->_accept_language = clsHTTP_Header::parse_language_header($language_header);
		}
	 ...
	 * @since   3.2.0
	 */
	public function send_headers(clsHTTP_Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		if ($response === NULL)
		{
                    // Default to the initial request message
                    $response = clsRequest::initial()->getHttpResponse();
		}
		...
		$processed_headers = array($protocol.' '.$status.' '.clsHttp_Response_Message::$messages[$status]);
		...
		foreach ($headers as $header => $value)
		{
			...
			$processed_headers[] = clsText::ucfirst($header).': '.$value;
		}

		if ( ! isset($headers['content-type']))
		{
			$processed_headers[] = 'Content-Type: '.WPFuel::$content_type.
				'; charset='.WPFuel::$charset;
		}
	...
	protected function _send_headers_to_php(array $headers, $replace)
	{
		...
		foreach ($headers as $key => $line)
		{
			if ($key == 'Set-Cookie' AND is_array($line))
			{
				...
				{
					clsCookie::set($name, $value['value'], $value['expiration']);
				}
	...
	public static function request_headers()
	{
		...
		if (function_exists('apache_request_headers'))
		{
			...
			return new clsHTTP_Header(apache_request_headers());
		}
		// If the PECL HTTP tools are installed
		elseif (extension_loaded('http'))
		{
			// Return the much faster method
			return new clsHTTP_Header(http_get_request_headers());
		}
		...
		return new clsHTTP_Header($headers);
	}
